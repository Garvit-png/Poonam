...
const buildFragment = (_iterations: number) => `#version 300 es
precision highp float;
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (direction === 'pingpong') {
        ...
        program.uniforms.iTime.value = fwd ? s * dur : (1 - s) * dur;
      } else {
    ...
    const onContextLost = (e: Event) => { e.preventDefault(); contextLost = true; cancelAnimationFrame(raf); };
    ...
    prefersReducedMotion ? renderStaticFrame() : (raf = requestAnimationFrame(loop));
    ...
    return () => {
      ...
      try { containerEl.removeChild(canvas); } catch (_) {}
